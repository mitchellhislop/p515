jQuery(document).ready( function($){
	...
	if( $('#em-date-start').length > 0 ){
		$("#em-date-start-loc").datepicker({
			altField: "#em-date-start", 
			altFormat: "yy-mm-dd",
			changeMonth: true,
			changeYear: true
		});
		$("#em-date-end-loc").datepicker({
		...
		if( $('#em-date-start').val() != '' ){
			date_dateFormat = $("#em-date-start-loc").datepicker('option', 'dateFormat');
			start_date_formatted = $.datepicker.formatDate( date_dateFormat, $.datepicker.parseDate('yy-mm-dd', $('#em-date-start').val()) );
			end_date_formatted = $.datepicker.formatDate( date_dateFormat, $.datepicker.parseDate('yy-mm-dd', $('#em-date-end').val()) );
		...
		$(".em-ticket-form, #em-tickets-form").each(function(i, el){
			...
			if(start.length > 0){
				start.first().datepicker({
					altField: el.find('.start').first(), 
					altFormat: "yy-mm-dd",
					changeMonth: true,
					changeYear: true
				});
		...
		$(".em-ticket-form, #em-tickets-form").each(function(i, el){
			el = $(el);
			start = el.find('.end-loc');
			if(start.length > 0){
				start.first().datepicker({
					altField: el.find('.end').first(), 
					altFormat: "yy-mm-dd",
					changeMonth: true,
					changeYear: true
				});
